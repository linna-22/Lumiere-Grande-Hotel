...
export default function PaymentStep({
  ...
  const subtotal = rooms.reduce((total, room) => {
    return total + Number(room.nightly_rate || 0) * nights;
  }, 0);
  ...
  const paymentOption = form.payment_option || "full";
  ...
  const generateKhqrPayment = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/payments/khqr/generate`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(
      () => {
        ...
        setPayment(null);
        setError("Payment window expired. Please generate a new KHQR code.");
      },
      5 * 60 * 1000,
    );
  ...
  }, [showQrModal, payment?.payment_id, API_BASE_URL, onContinue]);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          Select the payment option and payment method for this reservation.
        </p>
      </div>
      ...
      <div className="bg-base-800 border border-base-border rounded-xl p-5">
        ...
        <div className="mt-4 pt-4 border-t border-base-border space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-400">Room subtotal</span>
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-slate-200">-${discount.toFixed(2)}</span>
          </div>
        </div>
      </div>
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-white">50% Deposit</p>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <button
            type="button"
            onClick={() => handlePaymentMethodChange("bakong_khqr")}
